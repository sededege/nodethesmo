...
const CartItem = ({ item, setFlag, flag }) => {
  ...
  return (
    <div className="w-full p-4 px-2 rounded-lg shadow-md flex items-center gap-2">
      <img
        src={item && item[0].item.attributes.thumbnail.data.attributes.url}
        className="w-20 h-20 max-w-[60px]  object-contain"
        alt={item && item[0].item.attributes.name}
      />

      {/* name section */}
      <div className="flex flex-col gap-2">
        <p className="text-base text-[0.8rem] text-gray-500">
          {item[0].item.attributes.Title}
        </p>
        <div className="flex flex-col">
          <p className="text-[0.8rem] font-semibold">
         <span>Pack {item[0].item.attributes.qty} Unidades</span>
          </p>
         {/*  <p className="text-[0.8rem] font-semibold">
      ...
      <div className="group flex flex-col items-center  ml-auto cursor-pointer">
        ...
        <div className="flex mt-8">
          <p className="text-sm block text-gray-800 font-semibold">
            $ {parseFloat(Math.round(item[0].item.attributes.price*item[0].item.attributes.qty)) * qty}
          </p>
        </div>
      </div>
    </div>
