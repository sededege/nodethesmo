...
const Cardreward = ({ id, pointsuser, upd, number }) => {
  ...
  const UnitCounter = () => {
    ...
    const handleInputChange = (event) => {
      ...
      } }; return ( <div className="gap-2 flex w-full"> <button
      className="bg-btn text-white p-2 rounded-lg" onClick={() =>
      handleDecrement()}>-</button> <input className="w-[60px] text-center
      bg-transparent border-2 border-slate-500 rounded-lg text-white mx-auto"
      type="text" value={count2} onChange={(e) => handleInputChange(e)}
        />
