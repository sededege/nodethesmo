...
import Cratereward from "./Crateeward";
import Craterewardfinish from "./Craterewardfinish";
...
import Craterewardsol from "./Craterewardsol";
...
const Crate = () => {
  ...
  const [vjson, setValid] = useState(null);
  ...
  useEffect(() => {
    ...
    async function getCrate(req, res) {
      return new Promise((resolve, reject) => {
        fetch('http://localhost:3001/api/crates')
          .then(response => {
            res.statusCode = 200
    ...
    const isValidJSON = isJSONValid(data);
  }, [])
        ...
        <div className="w-full ">
          ...
          {dataall.length === 0 && (
            <div className="w-full h-full justify-center items-center flex">
              <p className="font text-white text-[2rem]">No Crates have been manifested yet</p>
asas
            </div>
          )}
          {/*  <div className="flex items-center justify-between mt-4 gap-2  ">
